...
function Ejercicio2() {
    ...
    reader.onload = (e) => {
        ...
        const onlywords = file.replace(/[^\w\s\d]/g, '').split(/\s+/);
    ...
    const count = () =>{
        if (ShowText) {
            const words = ShowText.split(/\b\w+\b/g).filter(word => word.trim() !== "");
            return words.length;
          } else {
            return 0;
          }
  ...
  return (
    <div className='text-center h-screen w-screen bg-orange-100 
        ...
        <div className='text-information p-4'>
            ...
            <h4 className='font-bold text-3xl text-rose-950'>Esta practica lee un archivo de texto plano como resultado muestra las palabras del texto: [/\b\w+\b/g]</h4>
